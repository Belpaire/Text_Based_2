...
def to_npquest(input,maxlen,word_to_hot_dict):
    ...
    for line in input:
        ...
        result=[]
        for word in line:
            if word not in word_to_hot_dict:
                result.append(word_to_hot_dict["UNK"])
            else:
                result.append(word_to_hot_dict[word])
...
def to_npanswer(input,word_to_hot_dict_answ,vocnba):
    ...
    for line in input:
        ...
        for word in line:
            #word=line[0]
            hot=0
            if word in word_to_hot_dict_answ:
                hot= word_to_hot_dict_answ[word]
            else:
                hot=word_to_hot_dict_answ["UNK"]
            index=np.argmax(hot)
            linenp[index]=1
...
if __name__ == '__main__':
    ...
    embedding_matrix300 = np.zeros((vocnbq + 1, EMBEDDING_DIM300))
    ...
    for line in f:
        ...
        word = values[0]
        coefs = np.asarray(values[1:], dtype='float32')
        embedding_index100[word] = coefs
    f.close()
    del f
    print('Found %s word vectors.' % len(embedding_index100))
    EMBEDDING_DIM100 = 100
    embedding_matrix100 = np.zeros((vocnbq + 1, EMBEDDING_DIM100))
    for word, i in word_to_hot_dict.items():
        embedding_vector = embedding_index100.get(word)
        if embedding_vector is not None:
            # words not found in embedding index will be all-zeros.
            i = np.argmax(i)
    ...
    for i in range(150):
        ...
        for linei in range(10):
            ...
            for index_for in range(len(line)):
                if line[index_for] not in word_to_hot_dict:
                    line[index_for]="UNK"
